...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	for(int i=0;i<tokens.size();++i)
	{
	        for(int j=0;j<topics.size();++j)
		{
			...
			if( ((nspace.substr(1) + "/" + tokens[i]).compare(topics[j].name) == 0) && (topics[j].datatype.compare("sensor_msgs/LaserScan") == 0) )
			{
	...
	if( (tmp_input_topics.size() != input_topics.size()) || !equal(tmp_input_topics.begin(),tmp_input_topics.end(),input_topics.begin()))
	{
		...
		if(input_topics.size() > 0)
		{
			...
			{
                scan_subscribers[i] = node_.subscribe<sensor_msgs::LaserScan> (input_topics[i].c_str(), 100, boost::bind(&LaserscanMerger::scanCallback,this, _1, input_topics[i]));
...
LaserscanMerger::LaserscanMerger()
{
	...
	point_cloud_publisher_ = node_.advertise<sensor_msgs::PointCloud2> (cloud_destination_topic.c_str(), 100, false);
	laser_scan_publisher_ = node_.advertise<sensor_msgs::LaserScan> (scan_destination_topic.c_str(), 100, false);
